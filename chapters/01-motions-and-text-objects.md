...
| `w b e`             | 下一个词/上一个词/词尾 | `word` / `backward` / `end`                             | `W B E` 忽略标点  |
| `f{char}` `F{char}` | 向前/后查找字符        | `forward`; 大写 `F` 为反向                              | `;` `,` 重复/反向 |
| `t{char}` `T{char}` | 到字符前/后            | `till`; 大写 `T` 为反向                                 | -                 |
| `0 ^ $`             | 行首/第一个非空/行尾   | `0` 代表开头; `^`/`$` 同正则的行首/行尾（`^` 跳过缩进） | `^` 常用          |

### 更大范围跳转

| 命令                  | 功能          | 记忆方法                                                        | 备注                   |
| --------------------- | ------------- | --------------------------------------------------------------- | ---------------------- |
| `gg` / `G`            | 文件首/尾     | `G` 可联想 Go to end; `gg` 为小写，大写的反向，跳到文件开头     | -                      |
| `{` / `}`             | 段落跳转      | 大括号 → 大块文本                                               | 长文快速浏览           |
| `(` / `)`             | 句子跳转      | 小括号 → 小块文本（句子）                                       | -                      |
| `Ctrl-o` / `Ctrl-i`   | 跳转回退/前进 | `Ctrl-o` 中，`o` 即 `older`                                     | 配合 `gg` `G` 练习往返 |
| `ma` -> `` ` `` / `'` | 标记跳转      | `m`(`mark`) 设置标记 a, `` `a `` 跳到标记精确位置 / `'a` 到行首 | a 可以是其它字符       |
